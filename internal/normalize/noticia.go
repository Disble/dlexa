...
// NoticiaNormalizer converts parsed noticia articles into lookup entries.
type NoticiaNormalizer struct{}

// NewNoticiaNormalizer returns a new normalizer for noticia articles.
func NewNoticiaNormalizer() *NoticiaNormalizer {
	return &NoticiaNormalizer{}
}

// Normalize converts parsed articles into canonical lookup entries.
func (n *NoticiaNormalizer) Normalize(ctx context.Context, descriptor model.SourceDescriptor, result parse.Result) (Result, error) {
	_ = ctx
	...
	for _, article := range result.Articles {
		if len(article.Sections) == 0 {
			return Result{}, model.NewProblemError(model.Problem{
				...
				Message:  fmt.Sprintf("noticia article has no sections to normalize for %q", descriptor.Name),
