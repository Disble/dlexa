...
func TestSearchMarkdownRendererRendersOrderedCandidatesAndEmptyState(t *testing.T) {
	...
	text := string(payload)
	for _, want := range []string{"## Resultado semántico para \"abu dhabi\"", "### 1. Abu Dhabi", "- sugerencia: `dlexa dpd Abu Dabi`", "### 2. Tilde en solo", "- More info: `dlexa espanol-al-dia solo`", "- nota: (not yet available as CLI command)", "### 3. Ruta rara", "- url: https://www.rae.es/archivo/ruta-rara", "- fallback_command: `dlexa search abu dhabi`", "## Problemas detectados", "- [academia] timeout"} {
