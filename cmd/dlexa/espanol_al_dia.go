...
func newEspanolAlDiaCommand(ctx context.Context, runtime runtimeRunner, format *string, noCache *bool) *cobra.Command {
	cmd := &cobra.Command{
		Use:           "espanol-al-dia <slug>",
		Short:         "Consulta un artículo específico de Español al día.",
		SilenceErrors: true,
		SilenceUsage:  true,
		Args:          cobra.MinimumNArgs(1),
		RunE: func(_ *cobra.Command, args []string) error {
			return runtime.RunModule(ctx, "espanol-al-dia", modules.Request{Query: strings.TrimSpace(strings.Join(args, " ")), Format: strings.TrimSpace(*format), NoCache: *noCache, Args: append([]string(nil), args...), Sources: []string{"espanol-al-dia"}})
		},
	}
	cmd.SetHelpFunc(func(*cobra.Command, []string) {
		_ = runtime.RenderHelp(ctx, model.HelpEnvelope{
			Command:     "dlexa espanol-al-dia",
			Summary:     "Consulta un artículo concreto de la superficie Español al día de la RAE.",
			Syntax:      "dlexa espanol-al-dia <slug>",
	...
	cmd.SetFlagErrorFunc(func(_ *cobra.Command, err error) error {
		return runtime.HandleSyntaxError(ctx, err, "dlexa espanol-al-dia <slug>")
	})
	cmd.Args = func(_ *cobra.Command, args []string) error {
		if len(args) == 0 {
			return runtime.HandleSyntaxError(ctx, fmt.Errorf("espanol-al-dia command requires an article slug"), "dlexa espanol-al-dia <slug>")
		}
		return nil
	}
	return cmd
